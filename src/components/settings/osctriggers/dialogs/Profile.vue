  ...
  <v-dialog v-model="model" width="50vw" max-width="512px">
    <v-card v-click-outside="closeDialog">
      <v-card-title v-if="mode === 'add'">
        {{ t('settings.osc.triggers.profile.dialog.title.add') }}
      </v-card-title>
      <v-card-title v-if="mode === 'edit'">
        {{ t('settings.osc.triggers.profile.dialog.title.edit') }}
      </v-card-title>
          ...
          <v-col :cols="12">
            ...
            >
              <v-text-field
                ...
                :label="t('settings.osc.triggers.profile.dialog.field_label')"
                :rules="[
                  rules.required,
                  rules.empty,
